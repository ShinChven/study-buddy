...
export const ArtifactPanel: React.FC<ArtifactPanelProps> = ({ messages, settings, onTakeTest, isOpen = false, onClose }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed lg:static inset-y-0 right-0 z-50 transform ${
        isOpen ? 'translate-x-0 w-[85vw] sm:w-80 border-l p-4 lg:p-6' : 'translate-x-full lg:translate-x-0 w-0 border-0 p-0 overflow-hidden'
      } transition-all duration-300 ease-in-out h-full bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 flex flex-col gap-6 lg:gap-8 lg:relative shadow-2xl lg:shadow-none shrink-0`}>
        
        {/* Close Button Mobile */}
        {isOpen && onClose && (
          <button 
            onClick={onClose}
            className="lg:hidden absolute top-4 right-4 p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors z-10"
          >
            <X size={20} />
          </button>
        )}

        <section className="flex flex-col min-h-0 flex-1 mt-8 lg:mt-0">
      ...
      <AnimatePresence>
        {showAllCards && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] flex items-center justify-center p-4 lg:p-8 backdrop-blur-md bg-slate-900/20 dark:bg-slate-900/60"
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[90vh] lg:max-h-[80vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800"
            ...
            >
              <div className="p-4 lg:p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-accent-50/30 dark:bg-accent-900/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg lg:text-xl font-bold text-slate-800 dark:text-slate-100">Knowledge Collection</h2>
                    <p className="text-xs lg:text-sm text-slate-500 dark:text-slate-400">{flipCards.length} cards collected</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowAllCards(false)}
              ...
              <div className="flex-1 overflow-y-auto p-4 lg:p-8 bg-white dark:bg-slate-900">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                  {flipCards.map((card, idx) => (
                    <motion.div
                      ...
                      className="bg-white dark:bg-slate-800 border border-accent-50 dark:border-accent-900/20 rounded-2xl p-5 lg:p-6 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-3 group border-b-4 border-b-accent-200 dark:border-b-accent-900"
                    >
                      <div className="flex items-center gap-2">
                        <span className="w-6 h-6 bg-accent-100 dark:bg-accent-900/40 text-accent-600 dark:text-accent-400 text-[10px] font-bold rounded-full flex items-center justify-center shrink-0">
                          {idx + 1}
                        </span>
                        <h3 className="font-bold text-accent-900 dark:text-accent-300 text-sm lg:text-base">{card.title}</h3>
                      </div>
              ...
              <div className="p-4 lg:p-6 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 flex justify-center">
                <button 
                  onClick={() => {
                    setShowAllCards(false);
                    onTakeTest();
                  }}
                  className="w-full lg:w-auto px-8 py-3 bg-accent-600 text-white font-bold rounded-xl hover:bg-accent-700 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-accent-200"
                >
                  <Trophy size={20} />
